...
//                         <li key={station.UNI_ID} style={{ marginBottom: '10px', border: '1px solid #ccc', padding: '10px', borderRadius: '13px',  }}>
...
export default function RegionSelector({ sidoName, selectedFuel }) {
    ...
    useEffect(() => {
        if (sigunList.length > 0) setSelectedSigunCode(sigunList[0].AREA_CD);
    }, [sigunList]);
    ...
    useEffect(() => {
        if (!selectedSigunCode || !selectedFuel) return;
    ...
    return (
        <div style={{
            ...
            <h2 style={{ marginBottom: '12px' }}>{sidoName} 지역별 저렴한 주유소 TOP 5</h2>
